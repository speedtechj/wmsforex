                ...
                \LaraZeus\Popover\Tables\PopoverColumn::make('virtual_invoice')
                    ...
                    ->content(function (Model $record) {
                        ...
                        if ($data_record) {
                            ...
                            $agents = Agentinvoices::where('manual_invoice', $record->virtual_invoice)
                                ->value('agent_id');
                            if ($agents) {
                                $agentname = Agent::where('id', $agents)->first();
                                
                                return new HtmlString('<span class="p-4">Invoice Not Encoded Invoice Issued to </span>'. $agentname->full_name);
                            } else {
